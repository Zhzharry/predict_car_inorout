...
class ModelPredictor:
    ...
    def predict_batch(self, X_batch):
        ...
        if self.selected_features is not None:
            # 检查特征数量是否匹配
            if X_batch.shape[1] < len(self.selected_features):
                raise ValueError(
                    f"特征数量不匹配: 输入特征数={X_batch.shape[1]}, "
                    f"需要特征数={len(self.selected_features)}"
                )
            # 检查索引是否超出范围
            max_idx = np.max(self.selected_features)
            if max_idx >= X_batch.shape[1]:
                raise ValueError(
                    f"特征索引超出范围: 最大索引={max_idx}, "
                    f"输入特征数={X_batch.shape[1]}"
                )
        ...
        if len(unique_predictions) == 1 and unique_predictions[0] == 0:
            print(f"警告: 所有预测结果都是0，可能存在问题")
            print(f"输入特征形状: {X_batch.shape}")
            print(f"模型特征数: {self.model.n_features_in_ if hasattr(self.model, 'n_features_in_') else '未知'}")
    ...
    def predict_test_file(self, test_filepath, output_filepath=None):
        ...
        if self.selected_features is not None:
            print(f"将使用特征索引数量: {len(self.selected_features)}")
            if hasattr(self.model, 'n_features_in_'):
                print(f"模型期望特征数: {self.model.n_features_in_}")
            # 检查索引范围
            if X_test.shape[1] > 0:
                max_idx = np.max(self.selected_features) if len(self.selected_features) > 0 else 0
                if max_idx >= X_test.shape[1]:
                    raise ValueError(
                        f"特征索引超出范围: 最大索引={max_idx}, "
                        f"输入特征数={X_test.shape[1]}"
                    )
